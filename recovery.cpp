...
main(int argc, char **argv) {
    ...
    if (update_rkimage) {
        ...
        if(strncmp(update_rkimage, "/mnt/sdcard", 11) == 0){
            int len = strlen(update_rkimage) + 20;
            char* modified_path = (char *)malloc(len);
            strlcpy(modified_path, update_rkimage, 6);
            strlcat(modified_path, "internal_sd", 18);
            strlcat(modified_path, update_rkimage+11, len);
            printf("(replacing path \"%s\" with \"%s\")\n",
                update_rkimage, modified_path);
            update_rkimage = modified_path;
        }
